...
const handleInvalidity = (field) => {
  const messageBox = field.parentElement.querySelector(
    `#${field.name}ErrorMsg`
  );
...
const inputFields = Array.from(
  form.querySelectorAll(".cart__order__form__question > input")
);
inputFields.map((field) =>
  field.addEventListener("change", () => checkForValidity(field))
);
  ...
  fetch(new URL(API_URL + "order"), {
    ...
    //   console.log(json);
      if (json.orderId) {
        ...
        newLocation.searchParams.set("orderId", json.orderId);
